...
export class UploadFinishedEvent extends CustomEvent<uploadFinishedDetail> {
  ...
  constructor(file: FirebaseUploadFile, isMultiple: boolean) {
    super(UploadFinishedEvent.eventName, {
      cancelable: true,
      detail: { file, isMultiple }
    });
  }
}
...
type FirebaseUploadFile = UploadFile & {
  /** storage reference */
...
export const Storage = <T extends MixinBase<BaseT>>(
  ...
  abstract class StorageClass extends superClass {
    ...
    _uploadFile(file: FirebaseUploadFile) {
      ...
      const fileRef = getFileRef(this.appName, this.storageBasePath, this.fileName || file.name, !isMulti, this.noFileExtension);
