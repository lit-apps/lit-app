...
type InternalLitInstance = {
  // This is a hack to avoid calling constructor 
  // instanciation work twice on HMR
  _initialize?: () => void;
  // _initConstructor?: () => void; // convention for our elements
  _$changedProperties?: Map<string, any> ;
}
...
function createPivotingClass(originalDefinition: Definition, tagName: string) {
  return class PivotCtor extends NativeHTMLElement {
    ...
    formAssociatedCallback() {
      const definition = definitionForElement.get(this);
      definition?.formAssociatedCallback?.apply(this, arguments as any);
    }
    formDisabledCallback() {
      const definition = definitionForElement.get(this);
      definition?.formDisabledCallback?.apply(this, arguments as any);
    }
    formResetCallback() {
      const definition = definitionForElement.get(this);
      definition?.formResetCallback?.apply(this, arguments as any);
    }
    formStateRestoreCallback() {
      const definition = definitionForElement.get(this);
...
function internalUpgrade(
  instance: HTMLElement & InternalLitInstance,
  originalDefinition: Definition,
  instancedDefinition: Definition,
  args?: any[]
) {
  ...
  if (ReflectApply(nativeNodeIsConnectedGetter, instance, [])) {
    try {
      ...
      console.warn('Error on HMR initial disconnectedCallback:', e.message);
    }
  }



  if(instance._initialize && instance._$changedProperties !== undefined) {
    // instance was already initialized
    instance._initialize = nullFn
  }
  // if(instance._initConstructor && instance._initConstructor !== nullFn) {
  //   // instance was already initialized
  //   instance._initConstructor = nullFn
  // }
}
